...
data_dir <- "E:/hyytiala2022"
data_dir_old <- "C:/Users/OMISTAJA/Documents/repos/Hyytiala_microclimates/data"
...
#sites 16 and 53 in 2021 data but not in 2022
all %>% filter(tomst_id == 94194358) # No such tomst_id in 2021 data, so it is fine
all %>% filter(tomst_id == 94194346) # No such tomst_id in 2021 data, so it is fine

# For sites 16 and 53 find 2021 data and copy to repository
f2 <- list.files(data_dir_old,
                 pattern = "data_", recursive = T, full.names = T)

# Copy site 8 data from last year data
f2[grepl("94194358", f2)]
dir.create(paste0(data_dir, "/16"))
file.copy(f2[grepl("94194358", f2)],
          paste0(data_dir, "/16/data_94194358_0.csv"))

# Copy site 53 data from last year data
f2[grepl("94194346", f2)]
dir.create(paste0(data_dir, "/53"))
file.copy(f2[grepl("94194346", f2)],
          paste0(data_dir, "/53/data_94194346_0.csv"))
